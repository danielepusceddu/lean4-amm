...
@[simp] theorem toNNRealReal_eq_toNNReal (x: ℝ+): ((x: ℝ≥0): ℝ) = (x: ℝ) := by rfl

@[simp] theorem toRealPReal_eq_toPReal (x: ℝ+):
  ⟨(x: ℝ), x.property⟩ = (x: ℝ+) := by rfl

theorem toNNReal_ne_zero (x: ℝ+): (x: ℝ≥0) ≠ 0 := by
  have h := x.property
  apply NNReal.ne_iff.mp
  rw [toNNRealReal_eq_toNNReal]
  exact ne_of_gt h

theorem zero_lt_toNNReal (x: ℝ+): 0 < (x: ℝ≥0) :=
  by exact x.property
...
@[simp] lemma add_toNNReal (x y: ℝ+):
  (((x+y): ℝ+): ℝ≥0) = (x:NNReal)+(y:NNReal) := by rfl
